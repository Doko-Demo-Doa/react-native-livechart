...
TGFX requires the New Architecture and a native development build; it cannot run in Expo Go or on React Native Web. Follow the install docs for [TGFX](https://tgfx.riteshshukla.in/api),
...
**React Native Web is not supported.** The chart relies on `react-native-tgfx` and
