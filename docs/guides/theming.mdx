...
<Frame caption="Light/dark themes and an accent-driven palette">
  <video autoPlay loop muted playsInline controls poster="/media/theming.jpg" src="/media/theming.mp4" />
</Frame>
...
Chart text (axes, badges, tooltips) uses the `font` prop. Point it at a
platform-registered font family:
...
TGFX's canvas-level custom-font registration is not yet exposed by LiveChart.
The legacy `typeface` and `fontManager` fields are accepted for source
