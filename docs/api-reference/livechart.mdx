...
<ParamField body="seriesOpacity" type="SharedValue<number>" default="1">
  ...
  It affects the line and area fill, candles and volume bars, value line, badge,
  and inline value text—not axes, reference lines, or scrub UI. While it is below
  `1`, the live dot and pulse ring fade completely out so the stroke cannot show
  through the dot; they fade back in after the multiplier returns to `1`.
...
<ParamField body="candleGaps" type="CandleGap[] | CandleGapsConfig" default="off">
  Explicit no-trade, unavailable, or unknown candle intervals. The library never
  infers their meaning from sparse timestamps and never inserts synthetic OHLC.
  See [Chart gaps](/guides/empty-candles).
</ParamField>

<ParamField body="lineGaps" type="ChartGap[] | ChartGapsConfig" default="off">
  Explicit no-trade, unavailable, or unknown line intervals. The line and every
  area fill stop at empty interval boundaries; bridge-enabled kinds draw an
  independently styled previous-value segment. Real samples take precedence.
  See [Chart gaps](/guides/empty-candles).
</ParamField>
...
<ParamField body="dot" type="boolean | DotConfig" default="true">
  ...
  `ring` (the crisp contrasting backing, on by default), opt-in `glow`, and
  `color`. `glow: true` enables restrained defaults, or pass a `DotGlowConfig`
  to tune `color`, `radius`, `blur`, and `opacity`. It is independent from
  the animated `pulse` prop and starts no animation clock. The same `DotConfig`
  is shared with `LiveChartSeries` (whose `dot` extends it with `pulse`,
  `valueLine`, and `valueLabel`). `dot={false}` is the canonical "hide" — the
  deprecated `dot={{ show: false }}` still works. With `timeScroll`,
  `trackWhileParked` keeps the dot on the **true live point** while scrolled
  back — it tracks the point's x, keeps its pulse, and hides once the point
  leaves the window (ignored with `badge.followViewEdge`). See
...
<ParamField body="axisAutoHideActive" type="SharedValue<boolean>">
  Keeps auto-hidden axes visible for as long as this reads `true` — for an
  external gesture (e.g. a custom price-axis drag) the chart itself doesn't see.
  **Experimental.**
</ParamField>

<ParamField body="xAxisAutoHideOpacityOut" type="SharedValue<number>">
  Mirrors the X-axis's own auto-hide opacity into this shared value instead of
  (or in addition to) animating the built-in axis, so external chart chrome
  (e.g. a bottom rail hosting the axis outside the canvas) can fade in sync.
  **Experimental.**
</ParamField>

<ParamField body="xAxisOffsetY" type="number" default="0">
  Vertical offset (px) applied to the X-axis line, ticks, and labels — moves
  them into an external bottom rail without affecting the plot padding.
  **Experimental.**
</ParamField>

<ParamField body="viewportResetKey" type="string | number">
  Resets any active pan/zoom (`timeScroll` / `zoom`) back to the live viewport
  whenever this key changes, without flashing auto-hidden axes the way a plain
  scroll-to-live would. **Experimental.**
</ParamField>
...
<ParamField body="static" type="boolean" default="false">
  ...
  small charts cheap when you render a list or grid of sparklines. You can toggle
  it at runtime; switching back to live restarts the suspended loops and catches
  up. Frame the data edge-to-edge with `timeWindow` + `nowOverride`. See the
...
<ParamField body="yRangeScale" type="SharedValue<number>" default="1">
  Manual Y-range multiplier around the fitted midpoint (`1` = auto-fit). Drive
...
<ParamField body="yRangeOffset" type="SharedValue<number>">
  Manual Y-range translation as a fraction of the fitted range's height — drive
  it from a vertical drag to pan the price scale. Applied after the fit and
  before `yRangeScale`. **Experimental.**
</ParamField>

<ParamField body="yRangeOverride" type="SharedValue<{ min: number; max: number } | null>">
  A fixed Y range for the whole run, replacing the fitted one outright — `null`
  falls back to auto-fit. The engine fits to what is visible and eases toward
  it, which squashes a series revealed left-to-right into a pre-sized window;
  a pinned range has nothing to ease toward. Assigned rather than eased, and
  bypasses `yRangeScale` / `yRangeOffset` / `nonNegative` / `maxValue` while set.
  **Experimental.**
</ParamField>

<ParamField body="yRangePanEnabled" type="SharedValue<boolean>">
  Unlocks vertical panning of the plot (writing `yRangeOffset`) via the same
  gesture that pans time horizontally. **Experimental.**
</ParamField>
...
<ParamField body="nowOverrideValue" type="SharedValue<number>">
  `nowOverride` as a shared value, so a caller sliding "now" every frame (e.g.
  a replay) can do so from the UI thread without re-rendering the chart. Takes
  precedence over `nowOverride` when both are given. **Experimental.**
</ParamField>

<ParamField body="omitTipBeyondData" type="boolean" default="false">
  End the line at its last real point instead of extending a flat tip to the
  right edge. The built-in "is this chart following live?" test can't tell a
  replay whose `nowOverride` is ahead of its data from a live chart; this opts
  such a chart out of the flat extension. No effect in candle mode.
  **Experimental.**
</ParamField>
...
<ParamField body="font" type="FontConfig">
  Font family / size / weight for all chart text. Use a platform-registered family;
  legacy custom-typeface fields are ignored by the TGFX renderer.
</ParamField>
