...
See [Line & area](/guides/line-and-area#dimming-replacement-data) or
[Multi-series](/guides/multi-series#dimming-replacement-data).
...
```ts
interface CandleGapBridgeStyle {
  color?: string;
  opacity?: number;
  strokeWidth?: number;
...
interface ScrubConfig {
  ...
  snapToMarkers?: boolean; // snap the crosshair to a marker timestamp within 2% of the visible window; default false
}
...
interface FontConfig {
  ...
  /** @deprecated Ignored by the TGFX renderer. */
...
TGFX uses native font-family resolution. Use a platform-registered `fontFamily`;
the legacy `typeface` and `fontManager` fields are retained only for source
